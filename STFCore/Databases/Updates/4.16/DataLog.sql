...
/****** Object:  Table [dbo].[SessionSummary]    Script Date: 10/23/2018 2:37:51 PM ******/
-- gparham: Changing the length of the 'Type' and 'Cycle' columns to 255.

IF EXISTS(SELECT 1 FROM sys.tables WHERE is_ms_shipped = 0 AND [name] = 'SessionSummary')
BEGIN
    ALTER TABLE SessionSummary ALTER COLUMN [Type] nvarchar(255) NULL
    ALTER TABLE SessionSummary ALTER COLUMN [Cycle] nvarchar(255) NULL
END
GO

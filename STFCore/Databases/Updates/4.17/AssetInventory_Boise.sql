...
-- bmyers 3/29/19 Populate FrameworkClient and FrameworkClientPlatform from old tables
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[VirtualMachineReservation]') AND type in (N'U'))
BEGIN
    INSERT INTO [dbo].FrameworkClient (FrameworkClientHostName, PowerState, UsageState, SessionId, Environment, PlatformUsage, HoldId, SortOrder,LastUpdated)
    SELECT VMName, PowerState, UsageState, SessionId, Environment, PlatformUsage, HoldId, SortOrder, LastUpdated
    FROM VirtualMachineReservation

    INSERT INTO FrameworkClientPlatform (FrameworkClientPlatformId, Name, Active)
    SELECT PlatformId, Name, Active
    FROM STFDATA01.EnterpriseTest.dbo.VirtualMachinePlatform

    INSERT INTO FrameworkClientPlatformAssoc (FrameworkClientHostName, FrameworkClientPlatformId)
    SELECT vmpa.Name, vmpa.PlatformId
    FROM STFDATA01.EnterpriseTest.dbo.VirtualMachinePlatformAssoc vmpa
    INNER JOIN FrameworkClient ON FrameworkClient.FrameworkClientHostName = vmpa.Name
    INNER JOIN FrameworkClientPlatform ON FrameworkClientPlatform.FrameworkClientPlatformId = vmpa.PlatformId
END
GO

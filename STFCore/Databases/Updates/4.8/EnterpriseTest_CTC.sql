...
-- CTC ONLY plugin settings
WITH P AS (SELECT * FROM (VALUES ('CertificateManagement'),('ComplexPrint'),('ConnectivityPrint'),('Discovery'),('DotOneX'),
                                 ('Ews'),('Firewall'),('IPConfiguration'),('IPSecurity'),('NetworkNamingServices'),
                                 ('Security'),('TelnetSnmp'),('WebProxy'),('WiFiDirect'),('Wireless')) as Plugins(PluginName)),
S AS (SELECT * FROM (VALUES ('ConnectivityShare'), ('EwsSiteMapLocation'), ('SeleniumChromeDriverPath'),
                            ('SeleniumIEDriverPath32'), ('SeleniumIEDriverPath64')) as Settings(SettingName)),
Plugin as (SELECT * FROM P CROSS JOIN S)
INSERT INTO SystemSetting
SELECT 'PluginSetting', PluginName, SystemSetting.Name, Value, Description
FROM SystemSetting
INNER JOIN Plugin on Plugin.SettingName = SystemSetting.Name
...
DELETE FROM SystemSetting
WHERE SubType = 'FrameworkSetting'
AND Name IN ('ConnectivityShare', 'EwsSiteMapLocation', 'SeleniumChromeDriverPath','SeleniumIEDriverPath32', 'SeleniumIEDriverPath64')
GO

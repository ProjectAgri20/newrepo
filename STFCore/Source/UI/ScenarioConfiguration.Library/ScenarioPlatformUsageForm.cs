...
{
    public partial class ScenarioPlatformUsageForm : Form
    {
        private SortableBindingList<ScenarioPlatformUsage> _platformUsage = null;
        private readonly EnterpriseScenario _scenario = null;
        ...
        private void MachineQuotaForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                user_TextBox.Text = UserManager.CurrentUserName;
                scenario_TextBox.Text = _scenario.Name;
                owner_TextBox.Text = _scenario.Owner;
            ...
            try
            {
                using (SmtpClient client = new SmtpClient(server))
                {
                    using (MailMessage message = new MailMessage())
                    {
                        ...
                        string body = Resource.MachineAssignmentIncrease.Replace("{USER}", UserManager.CurrentUserName);
                        body = body.Replace("{SCENARIO}", _scenario.Name);

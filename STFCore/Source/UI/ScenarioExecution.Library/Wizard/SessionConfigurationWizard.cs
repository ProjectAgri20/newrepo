    ...
    internal partial class SessionConfigurationWizard : Form
    {
        ...
        private void main_Wizard_Finish(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var page = summary_WizardPage.ContentArea.Controls[0] as WizardSessionInitializationPage;
                var schedule = page.StartSchedule;

                if (schedule == null || (schedule != null && !schedule.ScheduleEnabled))
                {
                    SessionClient.Instance.PowerUp(Ticket.SessionId);
                }
                else
                {
                    TraceFactory.Logger.Debug("Starting a scheduled powerup @ {0}".FormatWith(page.StartSchedule.StartDateTime));
                    SessionClient.Instance.PowerUp(Ticket.SessionId, page.StartSchedule);
                }

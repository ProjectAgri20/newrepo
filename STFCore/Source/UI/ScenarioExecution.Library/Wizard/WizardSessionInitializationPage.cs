...
namespace HP.ScalableTest.UI.SessionExecution.Wizard
{
    internal partial class WizardSessionInitializationPage : UserControl, IWizardPage
    {
        ...
        private SessionStartSchedule _schedule = new SessionStartSchedule();
        private SessionStartScheduleForm _scheduleStartForm = null;
        ...
        private void SessionMapElementReceived(object sender, SessionMapElementEventArgs e)
        {
            ...
            switch (element.ElementType)
            {
                ...
                case ElementType.Device:
                    //Adding cameras to the list of devices to be valided.
        ...
        public SessionStartSchedule StartSchedule
        {
            get { return _schedule; }
        }

        private void delayedStartLink_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (_scheduleStartForm == null)
            {
                _scheduleStartForm = new SessionStartScheduleForm(_schedule);
            }

            if (_scheduleStartForm.ShowDialog() == DialogResult.OK)
            {
                if (_schedule.ScheduleEnabled)
                {
                    delayedStartLink.Text = "Configure Delayed Session Start (ENABLED)";
                }
                else
                {
                    delayedStartLink.Text = "Configure Delayed Session Start (DISABLED)";
                }
            }
        }
    }

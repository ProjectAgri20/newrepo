...
namespace HP.ScalableTest.Plugin.DSSConfiguration.UIMaps
{
    public static class TopCatUiHelper
    {
        ...
        public static AutomationElementCollection GetChildren(Control parentControl, string childType)
        {
            ...
            return listBoxElement.FindAll(TreeScope.Children, new PropertyCondition(AutomationElement.ClassNameProperty, childType));
        }
        ...
        public static void PerformHumanAction(this List list, Action<List> action)
        {
            action(list);
            Thread.Sleep(HumanTimeSpan);
        }

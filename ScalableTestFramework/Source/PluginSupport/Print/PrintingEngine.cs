...
namespace HP.ScalableTest.PluginSupport.Print
{
    public class PrintingEngine
    {
        ...
        {
            return ProcessActivity(executionData, null);
        }
        ...
        /// </summary>
        /// <param name="executionData">Serialized activity data.</param>
        /// <param name="metadataConverters">The metadata converters to use when deserializing.</param>
        public PluginExecutionResult ProcessActivity(PluginExecutionData executionData, IEnumerable<IPluginMetadataConverter> metadataConverters)
        {
            PrintingActivityData data = null;
            if (metadataConverters == null)
            {
                data = executionData.GetMetadata<PrintingActivityData>();
            }
            else
            {
                data = executionData.GetMetadata<PrintingActivityData>(metadataConverters);
            }
            ...
            // Print the job
            var engine = new HP.ScalableTest.Print.PrintingEngine();
        ...
        public virtual StringBuilder PrintTag(PrintQueue printQueue, PluginExecutionData executionData)
        {
